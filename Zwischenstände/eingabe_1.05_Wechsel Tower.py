...
if os.path.exists(csv_file_path):
    ...
    if not df.empty:
        ...
        for index, row in df.iterrows():
            ...
            with col1:
                st.markdown(f"**Bestellnummer:** {row['Bestellnummer']} - **Status:** {row['Status']}")
            with col2:
                if row['Status'] == "nein":
                    if st.button(f"Abholen {row['Bestellnummer']}", key=row['Bestellnummer']):
                        bestellung_abholen(row['Bestellnummer'])
                        st.success(f"Bestellung {row['Bestellnummer']} wurde abgeholt!")
else:
